...
function legendDropdown() --clean up required?
	for _, p in pairs(game.players) do
		...
		if guiTop["EMC_frame"] == nil then
			...
			guiTop.add(type=button, name="close", caption="close", style)
			...
			if game.forces.player.technologies["electric-energy-distribution-1"].researched then
...
script.on_event(defines.events.on_gui_click, function(event) --test open/close for all?
		if event.element.name == "EMC_legend_Main" then
			...
			legendDropdown(event)
		elseif event.element.name == "EMC_frame" then --change to close button
			event.element.destroy()
			mainButton()
		end
	end
